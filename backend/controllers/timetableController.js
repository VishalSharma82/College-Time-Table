    ...
    try {
        ...
        group.classes.forEach((classEntry) => {
            classEntry.subjectsAssigned.forEach((assignment) => {
                const isLab = group.subjects.find(
        ...
        allAssignments.sort((a, b) => {
            const aTeacherWorkload = allAssignments.filter(assign => assign.teacher === a.teacher).length;
            const bTeacherWorkload = allAssignments.filter(assign => assign.teacher === b.teacher).length;
        ...
        for (let attempt = 0; attempt < attempts; attempt++) {
            ...
            if (success && scheduledCount === totalAssignments) {
                const finalTimetable = masterTimetable[group.classes[0].name].map((dayEntry) => ({
                    day: dayEntry.day,
                    slots: dayEntry.slots.slice(0, group.classes[0].periodsPerDay[dayEntry.day]),
                }));

                group.timetable = finalTimetable;
                await group.save();
                return res.json({ message: "Timetable generated ✅", timetable: finalTimetable });
            }
        }
...
exports.updateTimetable = async (req, res) => {
  try {
    ...
    if (req.group.classes && req.group.classes.length > 0) {
      req.group.timetable = { [req.group.classes[0].name]: timetable };
    } else {
      req.group.timetable = {};
    }
    await req.group.save();
