...
router.post('/', auth, requireRole('admin'), async (req, res) => {
  try {
    ...
    if (existing) {
      return res.status(400).json({ message: 'You already created a group with that name' });
    }
...
router.get('/mine', auth, requireRole('admin'), async (req, res) => {
  try {
    const groups = await Group.find({ owner: req.user.id }).populate('members', 'name email role');
    res.json(groups);
  } catch (err) {
...
router.get('/joined', auth, async (req, res) => {
  try {
    const groups = await Group.find({ members: req.user.id }).populate('owner', 'name email role');
    res.json(groups);
  } catch (err) {
...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    if (!isMember && !isOwner)
      return res.status(403).json({ message: 'Forbidden: not a member or owner' });
